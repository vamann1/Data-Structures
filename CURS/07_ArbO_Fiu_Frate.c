...
struct NodTree* inserare(struct NodTree* r, int cheie, int idParinte) {
	...
	if (!idParinte) { // nodul nou (cu cheie) va deveni noua radacina de arbore
		...
		if (p && !p->fiu)
			p->fiu = nou; // nodul nou devine prim fiu al nodului parinte p
		else {
			if (p)
			{
				if (!p->fiu->frate)
					...
					tmp->frate = nou; // nodul nou va fi ultimul in lista de frati ai lui p->fiu
				}
			}
			else
			{
				// nu exista nod parinte p cu cheie cautata
				// nodul nou deja alocat trebuie dezalocat pentru a evita mem leaks
				free(nou);
			}
		}
	}
