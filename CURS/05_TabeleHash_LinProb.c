...
char insertStudent(struct Student *ht, int size, struct Student s)
{
    ...
    {
        if (ht[i].name == NULL)
        { // verificare pozitie disponibila (name == NULL inseamna disponibil pentru scriere
            ht[i] = s;
...
int main()
{
    ...
    while (fgets(buffer, LINESIZE, f))
    {
        ...
        while (!insert)
        {                                                                           // proces realocare tabele de dispersie
            ...
            { // mutare elemente din vechea tabela pe noua tabela
                if (HTable[i].name)
